...
class BasicDataset(Dataset):
    def __init__(self, images_dir: str, masks_dir: str, classes_label: list, scale: float = 1.0, mask_suffix: str = ''):
    ...
    def preprocess(self, pil_img, scale, is_mask):
        ...
        if not is_mask:
            ...
            img_ndarray = mask2onehot(img_ndarray, self.classes_label)
    ...
    def __getitem__(self, idx):
        ...
        return {
            'image': torch.as_tensor(img.copy(), dtype=torch.float32),
            'mask': torch.as_tensor(mask.copy(), dtype=torch.float32)
        }
